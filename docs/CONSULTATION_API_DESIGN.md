...
- 실제 Supabase 환경변수로 API POST 성공 테스트
- 중복 제출 방어 최소 구현 여부 결정
- `/flow` 제출 연동은 그 다음 Phase로 분리 가능

## 15. Phase 4.2.3 구현 메모

Phase 4.2.3에서 Route Handler에 실제 Supabase 3테이블 insert를 연결합니다.

- `lib/consultation/repository.ts`를 생성하고 `saveConsultationRequest`를 구현합니다.
- Route Handler에서 `mapConsultationInputToStoragePayload`를 호출합니다.
- Route Handler에서 repository를 호출합니다.
- mock `request_id`를 제거하고 실제 `request_id`를 반환합니다.
- 저장 실패 시 500 응답과 `code: "SERVER_ERROR"`를 반환합니다.
- `/flow` 제출 연동은 아직 하지 않습니다.
- 실제 로컬 POST 성공 테스트에는 `.env.local` 설정이 필요합니다.
- events 실패 시 request/contact rollback 금지 정책을 유지합니다.

## 16. Phase 4.2.2-C 구현 메모
